...
import com.haigui.common.pdf.util.help.ChineseSymbolSplit;
 ...
 * <P>
 * A <CODE>PdfChunk</CODE> is a <CODE>PdfString</CODE> in a certain <CODE>PdfFont</CODE> and <CODE>BaseColor</CODE>.
 *
 * @see PdfString
 * @see Chunk
 * @see Font
	...
	PdfChunk split(float width) {
		...
		if (ft.getFontType() == BaseFont.FONT_TYPE_CJK && ft.getUnicodeEquivalent(' ') != ' ') {
			while (currentPosition < length) {
				...
				// }
				if (character == ' ' || ChineseSymbolSplit.isIncludeChar(character)) {
					lastSpace = currentPosition + 1;
					lastSpaceWidth = currentWidth;
				}
				...
				// 宽度超出时 判断下一个字符是否允许出现在首行 允许则退出 否则继续循环
				if (currentWidth > width){
					int nextPosition = currentPosition + 1;
					boolean hasNext = nextPosition < length;
					if (!hasNext || !ChineseSymbolSplit.isIncludeChar(valueArray[nextPosition])) {
						// 因为ChineseSymbolSplit里的字符都是可分割的 所以当lastSpace > splitPosition 且 splitPosition < 0 时 当前字符一定是属于ChineseSymbolSplit里的
						if (lastSpace > splitPosition && splitPosition < 0) {
