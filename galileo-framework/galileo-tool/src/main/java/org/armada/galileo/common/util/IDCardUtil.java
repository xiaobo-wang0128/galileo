...
public class IDCardUtil {
	...
	public static void main(String[] args) {
		...
		Boolean bool = isIDCard("211382198403215418");
	...
	public static boolean isIDCard(String certNo) {
		...
		for (int i = 0; i < cs.length; i++) {
			if (i == cs.length - 1 && (cs[i] == 'X' || cs[i] == 'x'))
				break;// 最后一位可以 是X或x
			if (cs[i] < '0' || cs[i] > '9')
				return false;
			if (i < cs.length - 1) {
	...
	public static String getSex(String identityNo) {
		...
		String sex = "";
		if (identityNo != null && identityNo.length() > 2) {
			int len = identityNo.length();
			String n = identityNo.substring(len - 2, len - 1);
	...
	public static String getSexCN(String identityNo) {

		if (CommonUtil.isEmpty(identityNo)) {
			return "";
		}
		String sex = "";
		if (identityNo != null && identityNo.length() > 2) {
	...
	public static Date getBirthday(String identityCard) {
		String birth = null;
		if (identityCard.length() == 15) {
			birth = "19" + identityCard.substring(6, 12);
		}
		else if (identityCard.length() == 18) {
			birth = identityCard.substring(6, 14);
		}
		else {
		...
		if (birth != null) {
			try {
				Date date = CommonUtil.parse(birth, "yyyyMMdd");
	...
	public static Integer getAge(String identityCard) {
		if (!IDCardUtil.isIDCard(identityCard)) {
			throw new BizException("身份证号不合法: " + identityCard);
		}
