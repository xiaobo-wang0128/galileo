...
public class BidiLine {
		...
		for (; currentChar < totalTextLength; ++currentChar) {
			...
			if (currentChar < totalTextLength - 1) {
				...
				if (width - charWidth - nextCharWidth < 0) {
					if (ChineseSymbolSplit.isIncludeChar((char)nextUnic)) {
	...
	public int trimRightEx(int startIdx, int endIdx) {
		...
		// 当下一个元素是标点时，前移一位
		//if ((idx+1 < text.length) && ChineseSymbolSplit.isIncludeChar(text[idx + 1]) && idx > 0) {
		//	idx--;
		//}
	...
	public int trimLeftEx(int startIdx, int endIdx) {
		...
		// 当前元素是标点时，前移一位
		//if (ChineseSymbolSplit.isIncludeChar(text[idx]) && idx > 0) {
		//	idx--;
		//}
