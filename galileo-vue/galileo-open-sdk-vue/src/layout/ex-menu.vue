...
var treeJson = [{
  ...
  "children": [{
      "label": "角色管理",
      "uri": $portal_context + "/page/system/user_role"
    },
    {
      "label": "用户管理",
      "uri": $portal_context + "/page/system/user_info"
    },
    {
      "label": "用户登陆日志",
      "uri": $portal_context + "/page/system/user_log"
    }
  ]
}];
  ...
  methods: {
    ...
    menuSelected(index, indexPath) {
      ...
      this.$emit('treeNodeClick', {
        ...
        uri: index,
        "openNew": _this.openNewMap[index]
      })
      ...
      for (var i = 0; i < tmpArray.length; i++) {
        for (var j = 0; j < tmpArray[i].children.length; j++) {
          ...
          tmpArray[i].children.forEach(e=>{
            ...
            _this.openNewMap[ e.uri ] = e.openNew ? true: false
