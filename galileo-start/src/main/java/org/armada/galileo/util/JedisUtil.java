...
//    }
    ...
    public void setnxex(CacheType cacheType, String key, String value, int expire) {
        Jedis j = null;
        try {
            j = pool.getResource();
            if (CommonUtil.isNotEmpty(password)) {
            ...
            String redisKey = new StringBuilder(cacheType.toString()).append("_").append(key).toString();
            j.getClient().sendCommand(ExtendCommand.SETNXEX, redisKey, value, String.valueOf(expire));
...
//    }
    ...
    private CopyOnWriteArrayList<String> autoExpireKeys = new CopyOnWriteArrayList<String>();

    /**
     * 自动过期的时间 (秒)
     */
    private int autoExpireSeconds = 180;

    {

        int sleepTime = (int) (autoExpireSeconds * 1000 * 0.5);

        Thread t = new Thread("reids_auto_expire") {
            public void run() {
                while (true) {
                    try {
                        ...
                        synchronized (log) {
                            if (autoExpireKeys.size() > 0) {
                                for (String redisKey : autoExpireKeys) {
                                    expire(redisKey, autoExpireSeconds);
                                }
                            }
                        }
                    } catch (Exception e) {
     ...
     * @param key
     */
    public void autoExpire(List<CacheType> cacheTypes, String key) {

        for (CacheType cacheType : cacheTypes) {
            String redisKey = new StringBuilder(cacheType.toString()).append("_").append(key).toString();
            expire(redisKey, autoExpireSeconds);
            synchronized (log) {
                if (autoExpireKeys.contains(redisKey)) {
                    continue;
                }
                autoExpireKeys.add(redisKey);
            }
        }
    }

    /**
     * 停止自动过期，但不删除 redis key
     *
     * @param cacheType
     * @param key
     */
    public void stopExpire(CacheType cacheType, String key) {
        synchronized (log) {
            String redisKey = new StringBuilder(cacheType.toString()).append("_").append(key).toString();
            autoExpireKeys.remove(redisKey);
            log.info("[redis] 停止自动过期:" + redisKey);
        }
    }
}
