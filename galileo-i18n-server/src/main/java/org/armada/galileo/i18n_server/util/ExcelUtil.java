...
public class ExcelUtil {
    ...
    public static ExcelData uploadExcel(HttpServletRequest request) {
        try {
        ...
        for (Row row : sheet) {
            ...
            if (rowIndex == 0) {
                ...
                for (int i = 0; i < columns; i++) {
                    ...
                    String value = getValue(cell, rowIndex, i);
                    if (value != null) {
                        value = value.trim();
                    }
            ...
            for (int i = 0; i < columns; i++) {
                ...
                String value = null;
                if (cell != null) {
                    value = getValue(cell, rowIndex, i);
                }
                if (value != null) {
                    value = value.trim();
                }
    ...
    private static String getValue(Cell cell, int row, int column) {
        ...
        if (cell.getCellType().equals(CellType.STRING)) {
            ...
            String err = CommonUtil.format("excel第 {} 行 {} 列 单元格格式不支持: {} , 请将excel中所有单元格格式设置为文本格式", row, column, cell.getCellType().toString());
    ...
    public static XSSFWorkbook getXSSFWorkbook(String sheetName, List<String> title, List<List<String>> values, boolean autoChangeType) {
    ...
    public static void export(OutputStream os, String sheetName, List<String> title, List<List<String>> values, boolean autoChangeType) {

        // 创建HSSFWorkbook
        // HSSFWorkbook wb = getHSSFWorkbook(sheetName, title, values, null);
        XSSFWorkbook wb = getXSSFWorkbook(sheetName, title, values, autoChangeType);
        // 响应到客户端
        try {
            wb.write(os);
            os.flush();
    ...
    public static void exportToWeb(String sheetName, List<String> title, List<List<String>> values, HttpServletResponse response) throws Exception {
        String fileName = sheetName + CommonUtil.format(new Date(), "yyyyMMddHHmmss") + ".xlsx";
        fileName = URLEncoder.encode(fileName, "UTF-8");
        response.setContentType("application/vnd.ms-excel;charset=utf-8");
        response.setHeader("Content-Disposition", "attachment;filename=" + fileName + ";" + "filename*=utf-8''" + fileName);

        ExcelUtil.export(response.getOutputStream(), "国际化词条", title, values, false);
    }
